	...
	public class VRGunHandler : VRInteractableItem 
	{
		...
		public bool SetLaserPointerEnabled
		{
			set
			{
				...
				laserPointerLine.enabled = laserPointerEnabled;
			}
		}
		...
		override protected void Init()
		{
			...
			foreach(AttachmentPrefabs attachmentPrefabs in attachmentPrefabs)
			{
				if (attachmentPrefabs.startLoaded && attachmentPrefabs.attachmentPrefab != null)
				{
					...
					{
						posMag = attachmentPrefabs.attachmentPrefab.GetComponentInChildren<VRMagazine>();
						posMag.AttachToGunHandler(attachmentPrefabs.attachmentReceiver);
					}
					if (posMag != null) currentMagazine = posMag;
				}
			}
        ...
        virtual public void Shoot()
        {
			...
			if (_selfBody != null)
			{
				if (_recoilKick < 0f || _angularRecoilKick < 0f || _recoilRecovery < 0f) SetDefaultRecoil();
				
				if (kickBackRoutine != null) StopCoroutine(kickBackRoutine);
				kickBackRoutine = StartCoroutine(kickBackRecovery());
				_selfBody.AddForce(-item.forward.normalized*_recoilKick/**Time.fixedDeltaTime*/, ForceMode.Impulse);//(heldBy.vrInput.isSteamVR() ? _recoilKick : _recoilKick*0.5f));
				_selfBody.AddTorque(-item.right.normalized*_angularRecoilKick/**Time.fixedDeltaTime*/, ForceMode.Impulse);//(heldBy.vrInput.isSteamVR() ? _angularRecoilKick : _angularRecoilKick*0.5f));
		...
		virtual protected void LoadBullet(bool slidePulled, bool onLoad = false)
		{
			...
			if (magazine != null) _hasBullet = magazine.TakeBullet();
			if (_hasBullet)
			{
		...
		public static void ApplyDecal(GameObject decalPrefab, RaycastHit hit)
		{
			...
			PoolingManager.instance.SetupDestroyInToReset(decal);
		...
		override protected Vector3 GetHeldPositionDelta()
		{
			if (usingSecondHeld && secondHeld != null && heldBy != null)
			{
				...
				if (secondHeldTransform == null)
				{
					...
					secondHeldTransform.localRotation = /*heldBy.isSteamVR() ? Quaternion.Euler(Vector3.right*-90f) : */Quaternion.identity;
				}
		...
		override public Quaternion GetLocalHeldRotation(VRInteractor hand)
		{
			...
			{
				Quaternion controllersDirection = Quaternion.LookRotation((secondHeld.heldBy.getControllerAnchor.position - hand.getControllerAnchorOffset.position).normalized);
				Vector3 secondHeldLocalPosition = secondHeld.GetLocalHeldPosition(secondHeld.heldBy);
				secondHeldLocalPosition.z = 0;
				Vector3 secondHeldControllerPos = VRUtils.TransformPoint(secondHeld.heldBy.getControllerAnchor.position, controllersDirection, secondHeld.heldBy.getControllerAnchor.lossyScale, secondHeldLocalPosition);
				Vector3 heldControllerPos = hand.getControllerAnchorOffset.position;
				Quaternion newRot = Quaternion.LookRotation(secondHeldControllerPos - heldControllerPos, hand.getControllerAnchorOffset.up);
				return newRot;
		...
		override protected Quaternion GetHeldRotationDelta()
		{
			...
			else return (heldByTransform.rotation*GetLocalHeldRotation(heldBy)) * Quaternion.Inverse(item.rotation);
		}
	}
}
#endif
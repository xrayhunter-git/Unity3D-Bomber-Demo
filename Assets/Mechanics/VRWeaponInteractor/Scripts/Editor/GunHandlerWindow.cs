...
namespace VRWeaponInteractor
{
	public class GunHandlerWindow : EditorWindow 
	{
		...
		static bool playOnFireFoldout = true;
		...
		private void ShowAttachmentReceiverSection(VRGunHandler.AttachmentPrefabs gunHandlerAttachmentPrefab, int gunHandlerAttachmentPrefabIndex, int gunHandlerAttachmentId)
		{
			...
			if (attachmentPrefab != null)
			{
				...
				} else
				{
					...
					var oldUseLaserPointer = attachment.useLaserSight;
					attachment.useLaserSight = EditorGUILayout.Toggle("Use Laser Sight", attachment.useLaserSight);
					if (oldUseLaserPointer != attachment.useLaserSight) changed = true;
					if (attachment.useLaserSight)
					{
						var oldLaserOrigin = attachment.laserPointOrigin;
						attachment.laserPointOrigin = ShowLaserPointerSettings(attachment.laserPointOrigin);
						if (attachment.laserPointOrigin != oldLaserOrigin) changed = true;
					}

					var oldUseShootDirection = attachment.useNewShootDirection;
					attachment.useNewShootDirection = EditorGUILayout.Toggle("Use New Shoot Direction", attachment.useNewShootDirection);
					if (oldUseShootDirection != attachment.useNewShootDirection) changed = true;
					if (attachment.useNewShootDirection)
					{
						EditorGUILayout.LabelField("Shoot Origin: " + attachment.newShootOrigin.ToString("N5"));
						EditorGUILayout.LabelField("Shoot Direction: " + attachment.newShootDirection.ToString("N5"));
						...
						if (shootOriginInstance == null || shootDirectionInstance == null)
						{
							if (GUILayout.Button("Setup Shoot Direction"))
							{
								...
								Undo.RegisterCreatedObjectUndo(shootOriginInstance, "Setup Shoot Direction");
								shootOriginInstance.transform.SetParent(gunHandler.item);
								shootOriginInstance.transform.localPosition = attachment.newShootOrigin;
								shootOriginInstance.transform.localScale *= gunHandler.item.localScale.magnitude*0.01f;
								shootDirectionInstance = GameObject.CreatePrimitive(PrimitiveType.Sphere);
								shootDirectionInstance.name = "Shoot Direction";
								Undo.RegisterCreatedObjectUndo(shootDirectionInstance, "Setup Shoot Direction");
								shootDirectionInstance.transform.SetParent(gunHandler.item);
								shootDirectionInstance.transform.localPosition = attachment.newShootOrigin + (attachment.newShootDirection*0.1f);
								shootDirectionInstance.transform.localScale = shootOriginInstance.transform.localScale;
								Selection.activeGameObject = shootDirectionInstance;
							}
						} else
						{
							var oldShotMode = attachment.newShotMode;
							attachment.newShotMode = (VRGunHandler.ShotMode)EditorGUILayout.EnumPopup("Shot Mode", attachment.newShotMode);
							if (oldShotMode != attachment.newShotMode) changed = true;
							switch(attachment.newShotMode)
							{
							case VRGunHandler.ShotMode.SINGLE_SHOT:
								break;
							case VRGunHandler.ShotMode.SHOTGUN_SPREAD:
								{
									var oldBulletsPerShot = attachment.newBulletsPerShot;
									attachment.newBulletsPerShot = EditorGUILayout.IntField("Bullets Per Shot", attachment.newBulletsPerShot);
									if (oldBulletsPerShot != attachment.newBulletsPerShot) changed = true;
									var oldConeSize = attachment.newConeSize;
									attachment.newConeSize = EditorGUILayout.FloatField("Cone Size", attachment.newConeSize);
									if (oldConeSize != attachment.newConeSize) changed = true;

									if (GUILayout.Button("Show Test Ray"))
									{
										Vector3 direction = (shootDirectionInstance.transform.position - shootOriginInstance.transform.position)*10;
										for(int j=0; j<attachment.newBulletsPerShot; j++) Debug.DrawRay(shootOriginInstance.transform.position, VRUtils.GetConeDirection(direction, attachment.newConeSize));
										SceneView.RepaintAll();
									}
								}
								break;
							case VRGunHandler.ShotMode.MACHINE_GUN_SPREAD:
								{
									...
									{
										Vector3 direction = (shootDirectionInstance.transform.position - shootOriginInstance.transform.position)*10;
										Debug.DrawRay(shootOriginInstance.transform.position, VRUtils.GetConeDirection(direction, attachment.newConeSize));
										SceneView.RepaintAll();
									}
								}
								break;
							...
							if (GUILayout.Button("Save"))
							{
								attachment.newShootOrigin = shootOriginInstance.transform.localPosition;
								attachment.newShootDirection = (shootDirectionInstance.transform.localPosition - shootOriginInstance.transform.localPosition)*10;
								Undo.DestroyObjectImmediate(shootOriginInstance);
								Undo.DestroyObjectImmediate(shootDirectionInstance);
								changed = true;
							}
					...
					var oldFireSound = attachment.newFireSound;
					attachment.newFireSound = (AudioClip)EditorGUILayout.ObjectField("New Fire Sound", attachment.newFireSound, typeof(AudioClip), false);
					if (oldFireSound != attachment.newFireSound) changed = true;
					var oldDryFireSound = attachment.newDryFireSound;
					attachment.newDryFireSound = (AudioClip)EditorGUILayout.ObjectField("New Dry Fire Sound", attachment.newDryFireSound, typeof(AudioClip), false);
					if (oldDryFireSound != attachment.newDryFireSound) changed = true;
